...
def new_rm(conn, *, table: str, key_column: str, page_size: int = 2, key_width: int = 4):
...
def test_ww_conflict_update_update_simulated():
    ...
    seed_if_absent(rm, xid=0, record={
        "flightNum": key,
        "price": 100,
        "numSeats": 150,
        "numAvail": 150,
    ...
    assert p2.err == ErrCode.VERSION_CONFLICT, f"expected VERSION_CONFLICT, got {p2.err}"
    rm.abort(2)
